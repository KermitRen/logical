function getDefaultPuzzle() {
    const puzzle = {height:7, width: 7, type:'nurikabe'}
...
function isBoringPuzzle(puzzle) {
    ...
    if(massPerIsland < 1.6 || massPerIsland >= 2.8 || variation >= 6 || longestIsland >= 8 || oneIslandsFraction >= 0.6) {
        return true
    }
...
function generatePuzzle(height, width) {
    ...
    for(let i = 0; i < maxIterations; i++) {
        ...
        let tries = 0
        let puzzle = null
        while((puzzle == null || isBoringPuzzle(puzzle)) && tries < maxTries) {
            puzzle = {type:'nurikabe', height:height, width:width, layout: createPlainTemplate(height, width)}
            setSolution(puzzle)
            tries++
        }
...
function generateNurikabe(settings) {
...
onmessage = function(protocol) {
    switch (protocol.data.protocol) {
        case 'generate': 
            const newPuzzle = generateNurikabe(protocol.data.settings)

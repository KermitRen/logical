...
function generatePuzzle(extraClues) {

    //Generate intial solution
    const puzzle = {type:'sudoku', height:9, width:9, layout: createPlainTemplate()}
    setSolution(puzzle)
    ...
    addRandomClues(puzzle, extraClues)

    return puzzle
}

function generateSudoku(settings) {
    const difficultySetting = (settings.find(setting => setting.label == 'difficulty')).options.find(option => option.set)

    switch (difficultySetting.val) {
        case 'easy': return generatePuzzle(6)
        case 'medium': return generatePuzzle(3)
        case 'hard': return generatePuzzle(0)
        default: return generatePuzzle(6)
    }
}
...
onmessage = function(protocol) {
    switch (protocol.data.protocol) {
        case 'generate': 
            const newPuzzle = generateSudoku(protocol.data.settings)
